...
```bash
mathmml-aws$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
mathmml-aws$ sam local start-api
mathmml-aws$ curl http://localhost:3000/
```
...
```bash
mathmml-aws$ sam logs -n NetCodeWebAPIServerless --stack-name mathmml-aws --tail
```
...
```bash
mathmml-aws$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name mathmml-aws
```
